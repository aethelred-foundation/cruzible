...
export function CruzibleLogo({
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      {showText && (
        <span
          className={`${textSize} font-display font-bold tracking-[0.2em] text-white`}
        >
          CRUZIBLE
        </span>
      )}
...
export function LiveDot({ color = "green", size = "sm" }: LiveDotProps) {
  ...
  return (
    ...
    >
      ...
      />
      <span
        className={`relative inline-flex rounded-full ${colorMap[color]} ${px} ${glowMap[color]}`}
      />
    </span>
  );
}
...
function buildSearchResults(query: string): SearchResultGroup[] {
  ...
  if (matchedValidators.length > 0) {
    results.push({
      ...
      items: matchedValidators
        .slice(0, 4)
        .map((v) => ({ label: v, href: "/validators" })),
    });
  }
      ...
      >
        ...
        <div className="flex items-center justify-between border-t border-slate-700/30 px-5 py-3">
          <div className="flex gap-4 text-xs text-slate-600">
            <span>
              <kbd className="mr-1 rounded-md border border-slate-700/50 bg-slate-800/50 px-1.5 py-0.5 text-slate-500">
                &uarr;&darr;
              </kbd>
              Navigate
            </span>
          ...
          <p className="text-xs text-slate-600">
            <kbd className="rounded-md border border-slate-700/50 bg-slate-800/50 px-1.5 py-0.5 text-slate-500">
              &#8984;K
            </kbd>
          </p>
        </div>
...
export function TopNav({ activePage }: TopNavProps) {
  ...
  return (
    <nav
      ...
      className="sticky top-0 z-50 border-b border-slate-800/30 bg-[#050810]/90 backdrop-blur-2xl"
    >
...
export function Footer() {
  return (
    <footer
      ...
      className="relative border-t border-slate-800/30 bg-[#050810]"
    >
      {/* Top accent */}
      ...
      <div className="relative mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        ...
        <div className="mb-12 flex flex-col items-start gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-md text-sm text-slate-500 leading-relaxed">
              The sovereign liquid staking protocol for the Aethelred AI
              verification network. TEE-verified, trustless, and built for
              scale.
            </p>
          </div>
          <div className="flex items-center gap-3">
        ...
        <div className="mt-16 flex flex-col items-center justify-between gap-4 border-t border-slate-800/30 pt-8 sm:flex-row">
          <div className="flex items-center gap-3">
            ...
            <p className="text-xs text-slate-600">
              &copy; {new Date().getFullYear()} Cruzible. Built by the Aethelred
              Foundation.
            </p>
          </div>
